...
//holding variables for right and left arrows on the keyboard
...
//Create variables to track the score
let score = 0;
...
function drawPaddle() {
    ...
    ctx.fillStyle = '#FF5733'; // Orange color for the paddle
    ctx.fill();
    ctx.closePath();
}
    ...
    for (c = 0; c < brickColumnCount; c++) {
        for (r = 0; r < brickRowCount; r++) {
            if (bricks[c][r].status === 1) {
                ...
                ctx.fillStyle = '#3498db'; // Blue color for the bricks
                ctx.fill();
                ctx.closePath();
            }
        }
...
function drawScore() {
    ctx.font = '18px Arial';
    ctx.fillStyle = '#FFD700'; // Gold color for the score
...
setInterval(draw, 10); //draw every 10 milliseconds
